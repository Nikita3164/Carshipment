...
form.addEventListener("submit", function(event) {
    ...
    for (index = 0, len = words_blacklist.length; index < len; ++index) {
            if (document.getElementById("id_comment").value.includes(words_blacklist[index])){
                messageElement.innerText = "Ваше сообщение похоже на спам";
                messageElement.classList.remove('msg_hidden');
                messageElement.classList.remove('msg_success');
                messageElement.classList.add('msg_error');
                flag = false; 
